...
DATA_DIR = Path(os.environ.get("DRAGON_DATA_DIR", str(BASE_DIR / "data")))
if not DATA_DIR.is_absolute():
    DATA_DIR = BASE_DIR / DATA_DIR
LOCAL_STORAGE_DIR = DATA_DIR / "storage"
LOCAL_METADATA_FILE = DATA_DIR / "metadata.json"
LOCAL_PROFILE_FILE = DATA_DIR / "profile.json"
...
def local_mode():
    """Use the filesystem when Firebase is not configured."""
    return FIREBASE_DB is None or FIREBASE_BUCKET is None
...
def local_read_json(path, default):
    try:
...
def local_write_json(path, value):
...
def local_assets():
    value = local_read_json(LOCAL_METADATA_FILE, {})
    return value if isinstance(value, dict) else {}


def local_set_asset(asset_id, data):
    assets = local_assets()
    assets[asset_id] = data
    local_write_json(LOCAL_METADATA_FILE, assets)


def local_update_asset(asset_id, update):
    assets = local_assets()
    if asset_id not in assets:
        return False
    assets[asset_id].update(update)
    local_write_json(LOCAL_METADATA_FILE, assets)
    return True


def local_delete_asset(asset_id):
    assets = local_assets()
...
def local_get_profile():
    value = local_read_json(LOCAL_PROFILE_FILE, {})
    return value if isinstance(value, dict) else {}


def local_set_profile(data):
    profile = local_get_profile()
    profile.update(data)
    local_write_json(LOCAL_PROFILE_FILE, profile)


def setup_firebase():
    global FIREBASE_DB, FIREBASE_BUCKET, FIREBASE_ERROR, FIREBASE_BUCKET_NAME
    ...
    try:
        ...
        project_id = service_account.get("project_id", "connecto-5814d")
...
def list_assets(include_private=False):
    if local_mode():
        assets = [
            asset_view(doc_id, data)
            for doc_id, data in local_assets().items()
...
def get_asset_by_id(asset_id):
    if local_mode():
        data = local_assets().get(asset_id)
    ...
    if local_mode():
        for doc_id, data in local_assets().items():
            if isinstance(data, dict) and data.get("slug") == slug:
                result = asset_view(doc_id, data)
                if include_private or result.get("status") == "published":
                    return result
                return None
        return None
...
def storage_read(path):
    if local_mode():
        if not path:
            return None
        try:
            local_path = (LOCAL_STORAGE_DIR / path).resolve()
            if LOCAL_STORAGE_DIR.resolve() not in local_path.parents:
                return None
...
def ensure_firebase():
    # Kept as a compatibility helper for the existing routes. Local mode is
    # intentionally valid and needs no paid storage or external account.
    return True


def readable_firebase_error(error):
    message = str(error).lower()
    if "bucket" in message or "storage" in message or "404" in message:
        return (
            "Firebase Storage upload fail hua. Firebase Console me Storage enable karein "
            "aur service-account project ka bucket check karein."
        )
    if "permission" in message or "403" in message or "forbidden" in message:
        return (
            "Firebase permission denied. FIREBASE_SERVICE_ACCOUNT_JSON isi project ka hona "
            "chahiye aur Firestore/Storage enabled hona chahiye."
        )
    if "firestore" in message or "database" in message:
        return "Firestore save fail hua. Firebase Console me Firestore Database create/enable karein."
    return "Asset save nahi ho paya. Render logs me original Firebase error check karein."
...
def global_template_data():
    ...
    return {
        "profile": profile,
        "firebase_ready": not local_mode(),
        "storage_mode": "local" if local_mode() else "firebase",
...
def new_asset():
    ...
    try:
        ...
        if local_mode():
            local_set_asset(asset_id, asset_data)
        else:
            asset_data["createdAt"] = firestore.SERVER_TIMESTAMP
            asset_data["updatedAt"] = firestore.SERVER_TIMESTAMP
            FIREBASE_DB.collection("assets").document(asset_id).set(asset_data)
        ...
        for path in uploaded_paths:
            try:
                ...
                app.logger.exception("Failed to clean up uploaded file")
...
def edit_asset(asset_id):
    ...
    if local_mode():
        local_update_asset(asset_id, update)
    else:
...
def toggle_asset(asset_id):
    ...
    update = {"status": new_status, "updatedAt": local_now()}
    if local_mode():
        local_update_asset(asset_id, update)
    else:
        update["updatedAt"] = firestore.SERVER_TIMESTAMP
...
def delete_asset(asset_id):
    ...
    if local_mode():
        local_delete_asset(asset_id)
        asset_folder = (LOCAL_STORAGE_DIR / f"assets/{asset_id}").resolve()
        if LOCAL_STORAGE_DIR.resolve() in asset_folder.parents:
...
def asset_detail(slug):
    ...
    if local_mode():
        local_update_asset(
            asset["id"],
            {"views": int(asset.get("views", 0) or 0) + 1},
        )
    elif FIREBASE_DB is not None:
        try:
...
def profile():
    if local_mode():
        profile_data = local_get_profile()
    elif FIREBASE_DB is None:
        profile_data = {}
    else:
...
def public_profile():
    if local_mode():
        public_data = local_get_profile()
    else:
        profile_snapshot = FIREBASE_DB.collection("settings").document("profile").get()
        public_data = profile_snapshot.to_dict() if profile_snapshot.exists else {}
